...
namespace FurionTemplate.Common.Redis
{
    public class NewLifeRedisCacheManager : IRedisCacheManager
    {
        ...
        {
            string redisConfiguration = App.Configuration["Redis:ConnectionString"];//获取连接字符串
        ...
        /// <summary>
        /// server=192.168.0.106:6379;db=1
        /// </summary>
        /// <param name="redisConfiguration"></param>
        public NewLifeRedisCacheManager(string redisConfiguration)
        {
            if (string.IsNullOrWhiteSpace(redisConfiguration))
            {
                throw new ArgumentException("redis config is empty", nameof(redisConfiguration));
        ...
        private FullRedis GetRedisConnection()
        {
            ...
            lock (redisConnectionLock)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("Redis服务未启用，请开启该服务", ex);
                }
            }
            return this.redisConnection;
        }
